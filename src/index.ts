...
app.post("/users", async (req: Request, res: Response) => {
  try {
    ...
    const user = new User(id, name, email, password, new Date().toISOString());
...
app.get("/accounts", async (req: Request, res: Response) => {
  try {
    ...
    const account: Account[] = accountsDB.map(
      (accountDB) =>
        new Account(
          accountDB.id,
          accountDB.balance,
...
app.post("/accounts", async (req: Request, res: Response) => {
  try {
    ...
    const account = new Account(id, 0, ownerId, new Date().toISOString());
